...
SIZE = 6
CELL_SIZE = 60  # ✅ تم تعديل حجم المربع فقط

# 0 = فارغ, 1 = لاعب 1 (أحمر), 2 = كمبيوتر (أزرق)
EMPTY, RED, BLUE = 0, 1, 2
AI_DEPTH = 3  # العمق الافتراضي للكمبيوتر
...
def computer_move():
    ...
    for move in get_all_moves(board, BLUE):
        score = minimax(move, AI_DEPTH, -math.inf, math.inf, False)  # ✅ استخدام AI_DEPTH مباشرة
        if score > best_score:
            best_score = score
            best_move = move
...
# الأرقام 1-6 أسفل كل عمود (يمين لليسار)
for i in range(SIZE):
...
# زر إعادة التشغيل
reset_btn = tk.Button(root, text="⟳ Reset Game", font=("Consolas", 16), command=reset_game)
...
# ===================== [إضافة وضع لاعب ضد لاعب] =====================
game_mode = "pvc"  # افتراضي ضد كمبيوتر
...
# أزرار اختيار وضعية اللعب
btn_pvp = tk.Button(root, text="Tow player", font=("Consolas", 16), command=lambda: set_game_mode("pvp"))
